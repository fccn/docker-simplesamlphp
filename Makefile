...
dev-image: ## Build the dev image
	docker build -t $(APP_NAME)_dev $(BUILD_ARGS) . --target=base

build-nc: ## Build the image without caching
	docker build --no-cache -t $(APP_NAME) $(BUILD_ARGS) .

shell: image ## Run a shell in a new container created from image
	docker run -a stdin -a stdout -i -t --rm --name "simplesamlphp_fpm" --entrypoint=/bin/sh $(APP_NAME)

dev-shell: dev-image ## Run a shell on dev container
		docker run -a stdin -a stdout -i -t --rm --name "simplesamlphp_fpm_dev" --entrypoint=/bin/sh $(APP_NAME)_dev

test: image ## Run simplesaml along side a webserver based on stvfccn/nginx_proxy image using docker-compose
	docker-compose --file test/docker-compose.yml up
  #docker run --rm --name "nginx_proxy" -v $(PWD)/test/config/sites-enabled:$(NGINX_OPTS)/sites-enabled -v $(PWD)/test/certs:/etc/nginx/ssl -v $(PWD)/test/webcontent:$(WEB_DOCUMENT_ROOT) -p $(TEST_HTTP_PORT):80 -p $(TEST_HTTPS_PORT):443 stvfccn/nginx_proxy
	#docker run --rm --name "simplesamlphp_fpm" -v $(PWD)/test/simplesaml:$(SAML_OPTS) $(APP_NAME)
